...
function boothTexture(proj: Project, dom: Domain): CanvasTexture {
  ...
  // Async-upgrade to real image if available
  if (proj.image_url) {
    const img = new Image(); img.crossOrigin = 'anonymous';
    img.onload = () => {
      cx.clearRect(0, 52, 512, 460);
      cx.drawImage(img, 0, 52, 512, 460);
...
export function buildWorld(projects: Project[]): void {
  ...
  domainIds.forEach(domId => {
    ...
    const edges = new LineSegments(
      ...
      new LineBasicMaterial({ color: new Color(dom.color) }),
    );
    ...
    const board = new Mesh(
      ...
      new MeshBasicMaterial({ map: makeZoneLabel(dom) }),
    );
